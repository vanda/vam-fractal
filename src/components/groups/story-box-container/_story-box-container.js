const initRevealer = () => {
  ...
  window.onload = () => {
    for (let i = 0; i < columnBoxes.length; i += 2) {
      ...
      const tallestInRow = textContentBoxes[(textContentBoxes[0].offsetHeight > textContentBoxes[1].offsetHeight ? 0 : 1)].offsetHeight;
  ...
  if (button) {
    button.onclick = () => {
      ...
      if (currentBoxes + 1 === boxes.length) {
        ...
        const textContentBoxes = columnBoxes.slice(currentBoxes - 1, currentBoxes + 2).map(el => el.querySelector('.b-story-box__content-text'));
        const tallestInRow = textContentBoxes[(textContentBoxes[0].offsetHeight > textContentBoxes[1].offsetHeight ? 0 : 1)].offsetHeight;
